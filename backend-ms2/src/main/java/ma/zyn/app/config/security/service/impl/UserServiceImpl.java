...
import ma.zyn.app.config.security.bean.ModelPermissionUser;
...
import ma.zyn.app.config.security.dao.facade.core.UserDao;
import ma.zyn.app.config.security.dao.specification.core.UserSpecification;
    ...
    private User createWithEnable(User t, boolean enable) {
        ...
        if (foundedUserByUsername != null || foundedUserByEmail != null) return null;
            ...
            */
			...
			else if(roleAsString.equals(AuthoritiesConstants.INFIRMIER)){
				saved = infirmierService.create(convertToInfirmier(t));
			}
 
 
 
 
			else if(roleAsString.equals(AuthoritiesConstants.DOCTOR)){
				saved = doctorService.create(convertToDoctor(t));
			}
 
			else if(roleAsString.equals(AuthoritiesConstants.PATIENT)){
				saved = patientService.create(convertToPatient(t));
			}
	 ...
	 private Infirmier convertToInfirmier(User user) {
        Infirmier item = new Infirmier();
        BeanUtils.copyProperties(user, item);
        return item;
    }
	 private Doctor convertToDoctor(User user) {
        Doctor item = new Doctor();
        BeanUtils.copyProperties(user, item);
        return item;
    }
	 private Patient convertToPatient(User user) {
